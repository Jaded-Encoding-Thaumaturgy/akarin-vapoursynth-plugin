...
JITGlobals *JITGlobals::get()
{
	static JITGlobals instance = [] {
		...
		// Set small code model so that calls will be rip-relative, rather than always movabs and call indirectly.
		// Verified that external functions will be called via PLT, so there shouldn't be any issues as long as
		// the memory mapper always allocate the code space within 2GB.
		// However, this breaks on macOS where the linker does not know how to create stubs.
...
{
	// LLVM build with LLVM_ENABLE_ABI_BREAKING_CHECKS will enforce that the error is checked
	// prior to calling get().
	if (!v) rr::abort("Unwrap(llvm::Expected<T>) failed.\n");
	return v.get();
}
template<typename T>
auto &Unwrap(T &&v)
